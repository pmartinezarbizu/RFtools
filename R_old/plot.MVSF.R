...
############################################
##plot the results of MVSF.test()
# x is a dataframe created by MVS.test()
...
plot(x$oob.err, seq(nrow(x), 1, -1), pch = 2, xlim = c(-1.5,1.2), ylim = c(0, nrow(x) + 1.5), yaxt = 'n', xaxt = 'n', bty = 'n', xlab = '', ylab = '', cex = 1.5)
...
cd = nrow(x):1
for(i in 1:nrow(x)){arrows(x$q05.null[cd[i]],nrow(x)-(i-1), x$q95.null[cd[i]],nrow(x)-(i-1),lwd=1.5,code=3,angle=90,length=0.04)}

#add species names
for(i in 1:nrow(x)) {text(-1.5, nrow(x) - (i-1), label = rownames(x)[i], font = 3, pos = 4, cex = 1)}
