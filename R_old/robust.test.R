...
#divide the dataset in a train (excluding 1 species) and a test dataset (with the excluded species)
...
rf <- randomForest::randomForest(class ~.,data=dattrain,proximity=TRUE,importance=TRUE,ntree= 1000,sampsize = c(rep(min(table(dattrain$class)), length(unique(dattrain$class)))))
...
res.test5 <- data.frame(class=predict(rf,dattest2), predict(rf, dattest2, type='p'))
...
# for each species calculate the lower 5% quantile of the empirical beta distribution
# : save in variable q5 From test3
q5 <- c()
for (i2 in 1:length(levels(dattrain$class))){
# list of probabilities of correct assigment for class i
prob.ca <- rfv2.train[art.correct.train==unique(dattrain$class)[i2],i2]
...
try(beta1 <- MASS::fitdistr(prob.ca,'beta',start=list(shape1=1,shape2=1)),silent=TRUE)
...
#extract from res.test3 the assigment for each species
# and calculate false positives 'fa'
#total assigments
ta <- c()
...
op <- res.test5[which(res.test5$class==levels(dattrain$class)[i3]),i3+1]
ta <- c(ta,length(op))
...
# add error columns
tot.assign <- cbind(tot.assign, assign.error = apply(tot.assign[,which(colnames(tot.assign)!='null')],sum,MARGIN=1,na.rm=T)/nMC)
false.pos <- cbind(false.pos, res.error = apply(false.pos[,which(colnames(false.pos)!='null')],sum,MARGIN=1,na.rm=T)/nMC)

rob.res <- list(assignment = data.frame(tot.assign),false.pos = data.frame(false.pos),false.pos.rate = data.frame(false.pos.rate),nMC = nMC)
