...
plot(1:length(c1),1:length(c1), xlim = c(min_POA-0.5,1.2),
     ylim = c(0, length(c1) + 1.5), yaxt = 'n', xaxt = 'n', bty = 'n', xlab = '', ylab = '',type='n')
...
points (sub_dat$p.assign,rep(n_spec,nrow(sub_dat)),pch=21,bg=col_sig,cex=1.5)
...
axis(1, at = c(min_POA,min_POA+((1-min_POA)/2),1), labels = c(min_POA,min_POA+((1-min_POA)/2),'1'), lwd.ticks = 1)
} else(stop('arg Species is not a valid class'))
}
