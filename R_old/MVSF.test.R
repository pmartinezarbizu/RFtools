...
rf <- randomForest::randomForest(target ~.,data=predictors,proximity=TRUE,importance=TRUE,ntree= 1000,sampsize = c(rep(min(table(target)), length(unique(target)))))

# Multivariate structure test
#shuffle species lables nMC times
# use paralellization
# save the error.rate for each species in variable rf.par
rf.par <- foreach::foreach(i = 1:nMC, .combine = 'cbind', .packages = 'randomForest') %dopar% randomForest(sample(target, length(target), replace = FALSE) ~., data = predictors, proximity = TRUE, importance = TRUE, ntree = 1000, sampsize = c(rep(min(table(target)), length(unique(target)))))$confusion[, length(unique(target))+1]
# close connections
closeAllConnections()
...
for (i in 1:length(unique(target))){
	p.par <- c(p.par, length(rf.par[i,] >= rf$confusion[, length(unique(target))+1][i])/(nMC+1))
}

# Flexibility test
# create nMC smooth observations per class
...
for (spec in 1:length(unique(target))) {
	nwrong[spec, 2] <-  1 - sum(ifelse(subset(res.t2, res.t2[, 1] == spec)[, 2] == spec, 1, 0))/nMC
} 

#####
# calculate the prob that the error rates of fake species and observed error rate are the same.
# use proportion test (proportion of errors)
p.sim <- c()
for (i in 1:length(unique(target))){
...
#write results
res2 = data.frame(oob.err = rf$confusion[,length(unique(target))+1],null.err = mean.par, P.null=p.par, q05.null=q05,q95.null=q95, smooth.err = nwrong[,2], p.smooth=p.sim)
